...
def setup_participant_directories(
    ...
    """
    src_participant_dir = os.path.join(
    ...
    if not os.path.exists(src_participant_dir):
        print(f"Source participant directory does not exist: {src_participant_dir}.")
    ...
    src_participant_audio_fnames = [f for f in os.listdir(src_participant_dir) if f.endswith('.wav')]
    
    # if not os.path.exists(output_participant_dir):
    #     os.makedirs(output_participant_dir, exist_ok=True)
    
    return src_participant_dir, output_participant_dir, src_participant_audio_fnames


def process_participant_audio_files(
    participant_id: int,
    participant_audio_fnames: List[str],
    participant_dir: str,
    ...
    Args:
        ...
        participant_audio_fnames: List of audio filenames with extension
...
def run_repp_analysis_for_participant(
    ...
    for recording_basename, recording_fname, stim_info_fname in audio_stim_pairs:
        ...
        parts = recording_basename.split("__")
        participant_id = int(parts[0].split("_")[1])
        node_id = int(parts[1].split("_")[1])
        trial_id = int(parts[2].split("_")[1])
        
        
        # NEW: Get config for this specific recording
        if callable(config):
            # If config is a function, call it with recording info
            ...
            # If config is an object, use it directly (backward compatible)
            recording_config = config
        ...
        results.append({
            'stim_name': stim_info[0]['stim_name'],
            'participant_id': participant_id,
            'node_id': node_id,
            'trial_id': trial_id,
            'audio_filename': recording_basename,
            'extracted_onsets': extracted_onsets,
            'stats': stats,
            'output': output,
            'stim_info_all': stim_info[0],
...
# Support functions for saving results, etc. can be added here
...
# ------------------------
# Load PKLs for a single task
# ------------------------
def load_task_dicts(base_pkl_dir, task_name):
    task_dir = os.path.join(base_pkl_dir, task_name)
    rows = []
...
# ------------------------
# Combine Task 1 + Task 2 into one CSV
# ------------------------
def combine_tasks_to_csv(base_pkl_dir, tasks, output_csv):

    # Load everything
    all_rows = []
    for task in tasks:
        all_rows.extend(load_task_dicts(base_pkl_dir, task))

    if not all_rows:
        print("No data found in any tasks.")
        return

    # Collect all columns
    all_keys = {k for row in all_rows for k in row}

    # Respect fixed column order
    first_cols = [c for c in FIRST_COLUMNS if c in all_keys]
    remaining_cols = sorted(all_keys - set(first_cols))
    fieldnames = first_cols + remaining_cols

    # Write combined CSV
    save_path = os.path.join(base_pkl_dir, output_csv)
